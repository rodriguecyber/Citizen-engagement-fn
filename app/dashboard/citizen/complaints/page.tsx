...
import { ComplaintDialog } from "@/components/dashboard/complaint-dialog"
...
const  fetchComplaints =async()=>{
  try {
    const res = await axios.get(`${API_URL}/complaints/citizen`,{
      headers:{
  ...
  return (
    ...
    <div className="space-y-6 ">
      <div className="flex flex-col justify-between space-y-4 md:flex-row md:items-center md:space-y-0">
        ...
        <Button asChild className="bg-primary text-white">
          <Link href="/dashboard/citizen/new-complaint">
            <FileText className="mr-2 h-4 w-4" /> New Complaint
          </Link>
        </Button>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card className="overflow-hidden">
          <CardHeader className="bg-blue-50 pb-3">
            <CardTitle className="flex items-center text-base">
              <Clock className="mr-2 h-5 w-5 text-blue-500" /> Ongoing Complaints
            </CardTitle>
          </CardHeader>
          <CardContent className="pt-4">
            <div className="text-3xl font-bold">{complaintCounts.received + complaintCounts.in_progress}</div>
        ...
        <Card className="overflow-hidden">
          <CardHeader className="bg-green-50 pb-3">
            <CardTitle className="flex items-center text-base">
              <CheckCircle className="mr-2 h-5 w-5 text-green-500" /> Resolved Complaints
            </CardTitle>
          </CardHeader>
          <CardContent className="pt-4">
            <div className="text-3xl font-bold">{complaintCounts.resolved}</div>
        ...
        <Card className="overflow-hidden">
          ...
          <CardContent className="pt-4">
            <div className="text-3xl font-bold">
              {complaintCounts.needs_info + complaintCounts.rejected + complaintCounts.escalated}
            </div>
            <p className="text-sm text-muted-foreground">Complaints needing your action</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="relative">
        ...
        <Input
          placeholder="Search complaints by title, ID, organization..."
          className="pl-10"
          value={searchQuery}
      ...
      <Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="mb-4 w-full overflow-x-auto">
          <TabsTrigger value="all" className="gap-2">
            ...
            All <Badge variant="outline">{complaintCounts.all.toString()}</Badge>
          </TabsTrigger>
          <TabsTrigger value="received" className="gap-2">
            <Clock className="h-4 w-4" />
            Received <Badge variant="outline">{complaintCounts.received.toString()}</Badge>
          </TabsTrigger>
          <TabsTrigger value="in_progress" className="gap-2">
            <Loader2 className="h-4 w-4" />
            In Progress <Badge variant="outline">{complaintCounts.in_progress.toString()}</Badge>
          </TabsTrigger>
          <TabsTrigger value="needs_info" className="gap-2">
            <MessageSquare className="h-4 w-4" />
            Needs Info <Badge variant="outline">{complaintCounts.needs_info.toString()}</Badge>
          </TabsTrigger>
          <TabsTrigger value="resolved" className="gap-2">
            <CheckCircle className="h-4 w-4" />
            Resolved <Badge variant="outline">{complaintCounts.resolved.toString()}</Badge>
          </TabsTrigger>
          <TabsTrigger value="rejected" className="gap-2">
            <XCircle className="h-4 w-4" />
            Rejected <Badge variant="outline">{complaintCounts.rejected.toString()}</Badge>
          </TabsTrigger>
          <TabsTrigger value="escalated" className="gap-2">
            <ArrowUpCircle className="h-4 w-4" />
            Escalated <Badge variant="outline">{complaintCounts.escalated.toString()}</Badge>
          </TabsTrigger>
        </TabsList>
      ...
      {selectedComplaint && (
        <ComplaintDialog complaint={selectedComplaint} isOpen={!!selectedComplaint} onClose={closeComplaintDetails} />
      )}
    </div>
    </div>
  )
